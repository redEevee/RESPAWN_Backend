...
public class UserGradeController {
    ...
    @GetMapping("/my-grade")
    public ResponseEntity<UserGradeResponse> myGrade(HttpSession session) {
        // session에서 buyerId 조회 후 buyer의 membershipTier 조회
        return ResponseEntity.ok(userGradeService.findBuyerGrade(getBuyerIdFromSession(session)));
    }
    ...
    public ResponseEntity<UserGradeResponse> gradeByBuyerId(@PathVariable Long buyerId) {
        return ResponseEntity.ok(userGradeService.findBuyerGrade(buyerId));
    }
    ...
    @PostMapping("/recalc")
    @Secured("ROLE_ADMIN")
    public ResponseEntity<GradeRecalcResponse> recalcMany(@RequestBody(required = false) GradeRecalcRequest request) {
